...
/**
 * Generates type-safe query classes from GraphQL Query type fields.
 *
 * <p>For each field in the Query type, this generator produces a class that:
 *
 * <ul>
 ...
 *   <li>Generates valid GraphQL query strings
 * </ul>
 *
 * <p>Example output for a query field:
 *
 * <pre>{@code
 * public final class GetUserQuery implements GraphQLOperation<UserDTO> {
 *     private final String id;
 *     private final UserProjection projection;
 *
 *     @Override
 *     public String operationName() { return "GetUser"; }
 *
 *     @Override
 *     public String toGraphQL() {
 *         return "query GetUser($id: ID!) { user(id: $id) " + projection.toGraphQL() + " }";
 *     }
 *
 *     @Override
 ...
 *
 *     @Override
 *     public Class<UserDTO> responseType() { return UserDTO.class; }
 *
...
public final class QueryGenerator {
  ...
  /**
   * Generates query classes for all fields in the Query type.
   *
  ...
  public List<JavaFile> generate() {
    ...
    for (FieldDefinition field : queryType.fields()) {
      files.add(generateQuery(field));
    }
  ...
  /**
   * Generates a query class for a single query field.
   *
   * @param field the field definition from the Query type
  ...
  public JavaFile generateQuery(@NotNull FieldDefinition field) {
    ...
    classBuilder.addMethod(
        MethodSpec.methodBuilder("builder")
            ...
            .addJavadoc("Creates a new builder for this query.\n\n@return a new builder\n")
            .build());

    // Add Builder class
    classBuilder.addType(
        generateBuilder(field, queryClassName, builderClassName, projectionClass, !isScalarReturn));
  ...
  private MethodSpec generateToGraphQLMethod(
    ...
    if (field.arguments().isEmpty()) {
      queryStart = "query " + operationName + " { " + field.name();
    } else {
      queryStart =
          "query " + operationName + "(" + varDefs + ") { " + field.name() + "(" + argRefs + ")";
    }

    if (hasProjection) {
      method.addStatement("return $S + \" \" + projection.toGraphQL() + \" }\"", queryStart);
    } else {
      method.addStatement("return $S + \" }\"", queryStart);
    }
