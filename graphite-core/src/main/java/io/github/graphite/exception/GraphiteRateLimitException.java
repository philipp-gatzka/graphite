 ...
 * <ul>
 *   <li>Client-side rate limiting when the local rate limiter is exhausted</li>
 *   <li>Server-side rate limiting when the server returns HTTP 429</li>
 * </ul>
 *
 * <p>The exception provides information about when the request can be retried:
 * <ul>
 *   <li>{@link #getRetryAfter()} - Duration to wait before retrying</li>
 *   <li>{@link #getResetTime()} - Absolute time when the rate limit resets</li>
 *   <li>{@link #getLimit()} - The rate limit ceiling</li>
 *   <li>{@link #getRemaining()} - Remaining requests in the current window</li>
 * </ul>
 *
 * <p>Example usage:
 * <pre>{@code
...
public class GraphiteRateLimitException extends GraphiteClientException {
     ...
     *
     * <p>This value is typically derived from the server's {@code Retry-After} header
     * or the client's rate limiter configuration.
     *
     ...
     *
     * <p>This value is typically provided by the server via rate limit headers
     * such as {@code X-RateLimit-Limit}.
     *
     ...
     *
     * <p>This value is typically provided by the server via rate limit headers
     * such as {@code X-RateLimit-Remaining}.
     *
