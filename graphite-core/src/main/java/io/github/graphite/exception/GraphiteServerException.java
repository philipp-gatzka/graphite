 ...
 *
 * <p>Server exceptions represent errors that occur on the server side, typically
 * indicated by HTTP error status codes (4xx or 5xx) or GraphQL-level errors in
 * the response. These are distinct from client exceptions, which represent errors
 * that occur before the request reaches the server.
 *
 * <p>This exception includes the HTTP status code when available, allowing callers
 * to implement status-specific error handling:
 * <ul>
 *   <li>4xx status codes indicate client errors (bad request, unauthorized, etc.)</li>
 *   <li>5xx status codes indicate server errors (internal error, service unavailable, etc.)</li>
 * </ul>
 *
 * <p>Common subclasses include:
 * <ul>
 *   <li>{@link GraphiteGraphQLException} - GraphQL-level errors in the response</li>
 * </ul>
 *
 * <p>Example usage:
 * <pre>{@code
...
public class GraphiteServerException extends GraphiteException {
    ...
    /**
     * Constructs a new server exception with the specified message, status code,
